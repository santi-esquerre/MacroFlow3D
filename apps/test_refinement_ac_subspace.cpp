...
void fill_bad_linear_gauge(const Grid3D& grid, PsptaInvariantField& inv) {
    ...
    for (size_t idx = 0; idx < n; ++idx) {
        psi1[idx] = modes[0][idx] + 0.15f * modes[2][idx];
        psi2[idx] = modes[1][idx];
    }
...
int main() {
    ...
    inv.resize(grid);
    fill_bad_linear_gauge(grid, inv);
    ...
    cfg.enabled = true;
    cfg.strategy = RefinementACStrategy::SubspaceQuadraticMap;
