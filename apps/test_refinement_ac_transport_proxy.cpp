...
void fill_transport_fragile_gauge(const Grid3D& grid, PsptaInvariantField& inv) {
    ...
    for (size_t idx = 0; idx < n; ++idx) {
        psi1[idx] = 0.45f * modes[0][idx] + 0.15f * modes[2][idx];
        psi2[idx] = 1.75f * modes[1][idx] - 0.10f * modes[3][idx];
    }
...
int main() {
    ...
    cfg.enabled = true;
    cfg.strategy = RefinementACStrategy::SubspaceQuadraticGaussNewtonProjectionProxy;
    ...
    assert(report.initial_projection.valid);
    assert(report.final_projection.valid);
    assert(report.final_quality.cross_product.rel_rms_mismatch <
           report.initial_quality.cross_product.rel_rms_mismatch);
    assert(report.final_projection.combined_score < report.initial_projection.combined_score);
