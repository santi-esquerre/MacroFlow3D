...
# --------------------------------------------------------------------
N_RUNS = 60  # macrodispersion_0..9
# FILE_PATTERN = "out1.00/macrodispersion_var1.00_{:d}.csv"  # ajusta si tu patrón difiere
FILE_PATTERN = "output/out_4.00/macrodispersion_var_v9_4.00_{:d}.csv"  # ajusta si tu patrón difiere
OUTFIG = "output/out_4.00/macrodispersion_avg.png"  # nombre del PNG de salida
...
for k in range(N_RUNS):
    ...
    if not path.is_file():
        sys.exit(f"ERROR: no se encontró «{fname}»")

    df = pd.read_csv(path)
    if not {"t", "Dx", "Dy", "Dz"} <= set(df.columns):
        sys.exit(f"ERROR: «{fname}» no contiene las columnas esperadas")
    dfs.append(df)
...
mean_data = (
    all_data.groupby("t", as_index=False)[["Dx", "Dy", "Dz"]].mean().sort_values("t")
)
mean_data[["Dx", "Dy", "Dz"]] /= 2 * lamb * vm * (2 / math.sqrt(math.pi))
mean_data["t"] /= lamb / vm

# # --------------------------------------------------------------------
# #   Puntos experimentales / de referencia
# # --------------------------------------------------------------------
# region VARIANZA 1
#  (x, y)  ->  bDL
# bDL = [
...
bx, by = zip(*bDT)
plt.scatter(
    bx, by, marker="^", facecolors="none", edgecolors="black", zorder=4, label="bDT"
)
# --------------------------------------------------------------------
...
plt.title("Promedio de la macrodispersión en función del tiempo (100 realizaciones)")
plt.legend()
plt.xscale("log")
plt.yscale("log")
plt.tight_layout()
...
# --------------------------------------------------------------------
# Gráfica adicional: Dy y Dz vs T en escala lineal
# # --------------------------------------------------------------------
# plt.figure(figsize=(8, 5))
# plt.plot(mean_data["t"], mean_data["Dy"], label=r"$\langle D_y\rangle$")
# plt.plot(mean_data["t"], mean_data["Dz"], label=r"$\langle D_z\rangle$")

# # Añadimos los puntos experimentales bDT
# bx, by = zip(*bDT)
# plt.scatter(
#     bx, by, marker="^", facecolors="none", edgecolors="black", zorder=4, label="bDT"
# )
...
# plt.xlabel("Tiempo $t$")
# plt.ylabel(r"Macrodispersión $\langle D_\alpha(t)\rangle$")
# plt.title("Macrodispersión transversal en función del tiempo (escala lineal)")
# plt.legend()
# plt.grid(True, alpha=0.3)
# plt.tight_layout()
# plt.savefig(OUTFIG.replace(".png", "_transversal_linear.png"), dpi=300)
# plt.show()
