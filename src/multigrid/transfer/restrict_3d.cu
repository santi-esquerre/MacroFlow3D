...
__global__ void restrict_kernel(real* __restrict__ phiCoarse, const real* __restrict__ phiFine,
...
void restrict_3d(CudaContext& ctx, const Grid3D& fine_grid, const Grid3D& coarse_grid,
                 DeviceSpan<const real> r_fine, DeviceSpan<real> b_coarse) {
    ...
    restrict_kernel<<<grid, block, 0, ctx.cuda_stream()>>>(b_coarse.data(), r_fine.data(), NX, NY,
                                                           NZ);
