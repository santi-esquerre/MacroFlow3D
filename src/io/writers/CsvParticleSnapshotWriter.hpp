...
public:
                      ...
                      const HostParticleSnapshot<T>& snap)
    {
        ...
        if (snap.has_status)      hdr += ",status";
        if (snap.has_wrap_counts) hdr += ",wrapX,wrapY,wrapZ";
        if (snap.has_unwrapped)   hdr += ",x_u,y_u,z_u";
        hdr += "\n";
        ...
        for (int i = 0; i < snap.n; i += s) {
            ...
            if (snap.has_wrap_counts && snap.wrapX)
                std::fprintf(f, ",%d,%d,%d",
                             snap.wrapX[i], snap.wrapY[i], snap.wrapZ[i]);
            if (snap.has_unwrapped && snap.x_u)
                std::fprintf(f, fmt_uw,
                             (double)snap.x_u[i],
                             (double)snap.y_u[i],
                             (double)snap.z_u[i]);
            std::fputc('\n', f);
