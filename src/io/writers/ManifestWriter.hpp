...
struct ManifestWriter {
     ...
     */
    static bool write(const OutputLayout& layout, const AppConfig& cfg, const GPUInfo& gpu) {
        ...
        // ── Grid ────────────────────────────────────────────────────
        j["grid"] = {{"nx", cfg.grid.nx},
                     {"ny", cfg.grid.ny},
                     {"nz", cfg.grid.nz},
                     {"dx", (double)cfg.grid.dx}};

        // ── Stochastic ──────────────────────────────────────────────
        j["stochastic"] = {{"sigma2", (double)cfg.stochastic.sigma2},
        ...
        // ── Transport ───────────────────────────────────────────────
        j["transport"] = {{"n_particles", cfg.transport.n_particles},
        ...
        j["analysis"]["macrodispersion"] = {
            {"enabled", mac.enabled},           {"NR", mac.NR},
            {"lambda", (double)mac.lambda},     {"vmean_norm", (double)mac.vmean_norm},
            {"sample_every", mac.sample_every}, {"var_estimator", mac.var_estimator}};

        const auto& snap = cfg.analysis.snapshots;
        j["analysis"]["snapshots"] = {
            {"enabled", snap.enabled}, {"every", snap.every}, {"stride", snap.stride}};
