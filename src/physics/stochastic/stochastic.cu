 ...
 */
__global__ void kernel_init_rng(curandState* __restrict__ states, const uint64_t base_seed,
                                const int n_modes) {
 ...
 */
__global__ void kernel_random_modes_gauss(curandState* __restrict__ states, real* __restrict__ V1,
                                          real* __restrict__ V2, real* __restrict__ V3,
                                          real* __restrict__ a, real* __restrict__ b,
                                          const real lambda, const int n_modes, const int k_max) {
    const int ix = threadIdx.x + blockIdx.x * blockDim.x;
    if (ix >= n_modes)
        return;
    ...
    while (flag == 1) {
        ...
        d = k * k * exp(-0.5 * k * k);
        if (curand_uniform_double(&localState) * 2.0 * exp(-1.0) < d)
            flag = 0;
    }
 ...
 */
__global__ void kernel_eval_logK(const real* __restrict__ V1, const real* __restrict__ V2,
                                 const real* __restrict__ V3, const real* __restrict__ a,
                                 const real* __restrict__ b, const int n_modes,
                                 real* __restrict__ logK, const real h, const int nx, const int ny,
                                 const int nz, const real sigma_f) {
    const int ix = threadIdx.x + blockIdx.x * blockDim.x;
 ...
 */
__global__ void kernel_exp(real* __restrict__ K, const real* __restrict__ logK, const real log_Kg,
                           const size_t n) {
...
__global__ void kernel_minmax_sum(const real* __restrict__ data, const size_t n,
                                  real* __restrict__ block_mins, real* __restrict__ block_maxs,
...
void init_stochastic_rng(StochasticWorkspace& workspace, uint64_t seed, const CudaContext& ctx) {
    ...
    kernel_init_rng<<<grid, block, 0, ctx.cuda_stream()>>>(workspace.rng_states.data(), seed,
                                                           workspace.n_modes);
...
void generate_gaussian_field(StochasticWorkspace& workspace, const Grid3D& grid,
    ...
    // === Stage 2: Evaluate spectral sum at each grid cell ===
    {
        ...
        dim3 block(8, 8, 8);
        dim3 grid_dim((grid.nx + block.x - 1) / block.x, (grid.ny + block.y - 1) / block.y,
                      (grid.nz + block.z - 1) / block.z);

        kernel_eval_logK<<<grid_dim, block, 0, ctx.cuda_stream()>>>(
            workspace.k1.data(), workspace.k2.data(), workspace.k3.data(), workspace.coef_a.data(),
            workspace.coef_b.data(), n_modes, workspace.logK.data(), grid.dx, grid.nx, grid.ny,
            grid.nz, sigma_f);
...
void generate_K_lognormal(DeviceSpan<real> K, DeviceSpan<const real> logK, const Grid3D& grid,
                          const StochasticConfig& cfg, const CudaContext& ctx) {
...
void generate_K_field(DeviceSpan<real> K, StochasticWorkspace& workspace, const Grid3D& grid,
                      const StochasticConfig& cfg, const CudaContext& ctx) {
    ...
    generate_K_lognormal(K, DeviceSpan<const real>(workspace.logK.data(), workspace.n_cells), grid,
                         cfg, ctx);
}

void compute_field_stats(DeviceSpan<const real> data, real& min_val, real& max_val, real& mean_val,
    ...
    kernel_minmax_sum<<<n_blocks, block, smem_size, ctx.cuda_stream()>>>(
        const_cast<real*>(data.data()), n, d_mins.data(), d_maxs.data(), d_sums.data());
