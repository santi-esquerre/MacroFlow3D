...
struct ScalarField {
    ...
    explicit ScalarField(const Grid3D& grid)
        : nx(grid.nx), ny(grid.ny), nz(grid.nz), dx(grid.dx), dy(grid.dy), dz(grid.dz),
          data(grid.num_cells()) {}
...
struct VelocityField {
    ...
    size_t idx_U(int i, int j, int k) const {
        return static_cast<size_t>(i) + static_cast<size_t>(j) * stride_U_j() +
               static_cast<size_t>(k) * stride_U_k();
    }
    size_t idx_V(int i, int j, int k) const {
        return static_cast<size_t>(i) + static_cast<size_t>(j) * stride_V_j() +
               static_cast<size_t>(k) * stride_V_k();
    }
    size_t idx_W(int i, int j, int k) const {
        return static_cast<size_t>(i) + static_cast<size_t>(j) * stride_W_j() +
               static_cast<size_t>(k) * stride_W_k();
    }
 ...
 *               the rest stay at 0.
 */
enum class VelocityLayout : uint8_t { CompactMAC = 0, PaddedFaceField = 1 };
...
struct PaddedVelocityField {
    ...
    PaddedVelocityField(int nx_, int ny_, int nz_, real dx_ = 1.0, real dy_ = 1.0, real dz_ = 1.0)
        : nx(nx_), ny(ny_), nz(nz_), dx(dx_), dy(dy_), dz(dz_) {
        allocate();
    }
    ...
    size_t field_size() const { return static_cast<size_t>(nx + 1) * (ny + 1) * (nz + 1); }
    ...
    size_t merge_id(int ix, int iy, int iz) const {
        return static_cast<size_t>(iz) * (ny + 1) * (nx + 1) + static_cast<size_t>(iy) * (nx + 1) +
               ix;
    }
