 ...
 */
__global__
void kernel_velocity_diagnostics(
    const real* __restrict__ U,
    const real* __restrict__ V,
    const real* __restrict__ W,
    real* __restrict__ div_out,
    real* __restrict__ omag_out,
    real* __restrict__ hel_out,
    ...
    bool compute_curl)  // false if any dim < 3
{
    ...
    const real div_val = (Uip1 - Ui) * inv_dx
                       + (Vjp1 - Vj) * inv_dy
                       + (Wkp1 - Wk) * inv_dz;
    div_out[idx] = div_val;
    ...
    // uc at (i, j, k±1)
    const real uc_kp1 = real(0.5) * (U[padded_idx(i, j, k + 1, nx, ny)]
                                   + U[padded_idx(i + 1, j, k + 1, nx, ny)]);
    const real uc_km1 = real(0.5) * (U[padded_idx(i, j, k - 1, nx, ny)]
                                   + U[padded_idx(i + 1, j, k - 1, nx, ny)]);
    // uc at (i, j±1, k)
    const real uc_jp1 = real(0.5) * (U[padded_idx(i, j + 1, k, nx, ny)]
                                   + U[padded_idx(i + 1, j + 1, k, nx, ny)]);
    const real uc_jm1 = real(0.5) * (U[padded_idx(i, j - 1, k, nx, ny)]
                                   + U[padded_idx(i + 1, j - 1, k, nx, ny)]);

    // vc at (i±1, j, k)
    const real vc_ip1 = real(0.5) * (V[padded_idx(i + 1, j, k, nx, ny)]
                                   + V[padded_idx(i + 1, j + 1, k, nx, ny)]);
    const real vc_im1 = real(0.5) * (V[padded_idx(i - 1, j, k, nx, ny)]
                                   + V[padded_idx(i - 1, j + 1, k, nx, ny)]);
    // vc at (i, j, k±1)
    const real vc_kp1 = real(0.5) * (V[padded_idx(i, j, k + 1, nx, ny)]
                                   + V[padded_idx(i, j + 1, k + 1, nx, ny)]);
    const real vc_km1 = real(0.5) * (V[padded_idx(i, j, k - 1, nx, ny)]
                                   + V[padded_idx(i, j + 1, k - 1, nx, ny)]);

    // wc at (i±1, j, k)
    const real wc_ip1 = real(0.5) * (W[padded_idx(i + 1, j, k, nx, ny)]
                                   + W[padded_idx(i + 1, j, k + 1, nx, ny)]);
    const real wc_im1 = real(0.5) * (W[padded_idx(i - 1, j, k, nx, ny)]
                                   + W[padded_idx(i - 1, j, k + 1, nx, ny)]);
    // wc at (i, j±1, k)
    const real wc_jp1 = real(0.5) * (W[padded_idx(i, j + 1, k, nx, ny)]
                                   + W[padded_idx(i, j + 1, k + 1, nx, ny)]);
    const real wc_jm1 = real(0.5) * (W[padded_idx(i, j - 1, k, nx, ny)]
                                   + W[padded_idx(i, j - 1, k + 1, nx, ny)]);
    ...
    const CudaContext&         ctx)
{
    ...
    kernel_velocity_diagnostics<<<(unsigned)n_blocks, block, 0, ctx.cuda_stream()>>>(
        vel.U_ptr(), vel.V_ptr(), vel.W_ptr(),
        diag.divergence.device_ptr(),
        diag.vorticity_mag.device_ptr(),
        diag.helicity.device_ptr(),
        nx, ny, nz,
