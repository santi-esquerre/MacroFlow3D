...
__global__ void kernel_init_head_linear_x(real* __restrict__ h, int nx, int ny, int nz, real h_west,
                                          real h_east) {
...
void init_head_guess(DeviceSpan<real> h, const Grid3D& grid, const BCSpec& bc,
    ...
    if (bc.xmin.type == BCType::Dirichlet && bc.xmax.type == BCType::Dirichlet) {
        ...
        kernel_init_head_linear_x<<<grid_dim, block, 0, ctx.cuda_stream()>>>(
            h.data(), grid.nx, grid.ny, grid.nz, bc.xmin.value, bc.xmax.value);
    } else {
        // No obvious guess; use zero
        int block = 256;
        int grid_1d = (h.size() + block - 1) / block;
        kernel_fill_value<<<grid_1d, block, 0, ctx.cuda_stream()>>>(h.data(), h.size(), real(0.0));
    }
    MACROFLOW3D_CUDA_CHECK(cudaGetLastError());
}

HeadSolveResult solve_head(DeviceSpan<real> h, DeviceSpan<const real> K, const Grid3D& grid,
                           const BCSpec& bc, const HeadSolveConfig& cfg, CudaContext& ctx,
    ...
    MACROFLOW3D_CUDA_CHECK(cudaMemcpyAsync(hier.levels[0].x.data(), h.data(), n * sizeof(real),
                                           cudaMemcpyDeviceToDevice, ctx.cuda_stream()));

    // === 5. Solve based on solver type ===
    if (cfg.solver_type == HeadSolverType::MG) {
        // Standalone MG V-cycles
        VCycleResult vcycle_result = mg_solve(ctx, hier, mg_cfg, bc, cfg.mg_max_cycles, cfg.rtol);
        ...
        solvers::CGResult cg_result =
            solvers::cg_solve(ctx, A_pos, DeviceSpan<const real>(workspace.rhs.span()),
                              hier.levels[0].x.span(), cg_cfg, workspace.cg_workspace);
        ...
        solvers::PCGResult pcg_result =
            solvers::pcg_solve(ctx, A, M, DeviceSpan<const real>(workspace.rhs.span()),
                               h, // Use h directly as solution
                               pcg_cfg, workspace.pcg_workspace);
    ...
    MACROFLOW3D_CUDA_CHECK(cudaMemcpyAsync(h.data(), hier.levels[0].x.data(), n * sizeof(real),
                                           cudaMemcpyDeviceToDevice, ctx.cuda_stream()));
