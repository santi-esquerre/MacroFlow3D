 ...
 */
__global__ void coarsen_K_kernel(real* __restrict__ K_coarse, const real* __restrict__ K_fine,
    ...
    for (int IZ = 0; IZ < NZ; ++IZ) {
        ...
        real log_sum = log(K_fine[idx000]) + log(K_fine[idx100]) + log(K_fine[idx010]) +
                       log(K_fine[idx110]) + log(K_fine[idx001]) + log(K_fine[idx101]) +
...
void coarsen_K(CudaContext& ctx, const Grid3D& coarse_grid, const Grid3D& fine_grid,
    ...
    coarsen_K_kernel<<<grid, block, 0, ctx.cuda_stream()>>>(K_coarse.data(), K_fine.data(), NX, NY,
                                                            NZ);
