 ...
 */
__device__ __forceinline__ real harmonic_mean(real Ka, real Kb) {
 ...
 */
__device__ __forceinline__ int cell_idx(int i, int j, int k, int nx, int ny) {
 ...
 */
__device__ __forceinline__ int U_idx(int i, int j, int k, int nx, int ny) {
 ...
 */
__device__ __forceinline__ int V_idx(int i, int j, int k, int nx, int ny) {
 ...
 */
__device__ __forceinline__ int W_idx(int i, int j, int k, int nx, int ny) {
    return i + j * nx + k * nx * ny;
}
...
__host__ __device__ __forceinline__ uint8_t bc_to_int(BCType t) {
    switch (t) {
    case BCType::Neumann:
        return BC_NEUMANN;
 ...
 */
__global__ void kernel_U_interior(real* __restrict__ U, const real* __restrict__ H,
                                  const real* __restrict__ K, int nx, int ny, int nz, real dx) {
 ...
 */
__global__ void kernel_V_interior(real* __restrict__ V, const real* __restrict__ H,
                                  const real* __restrict__ K, int nx, int ny, int nz, real dy) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int face_j = 1 + threadIdx.y + blockIdx.y * blockDim.y;
    int k = threadIdx.z + blockIdx.z * blockDim.z;

    if (i >= nx || face_j > ny - 1 || k >= nz)
        return;
 ...
 */
__global__ void kernel_W_interior(real* __restrict__ W, const real* __restrict__ H,
                                  const real* __restrict__ K, int nx, int ny, int nz, real dz) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    int face_k = 1 + threadIdx.z + blockIdx.z * blockDim.z;

    if (i >= nx || j >= ny || face_k > nz - 1)
        return;
 ...
 */
__global__ void kernel_U_west(real* __restrict__ U, const real* __restrict__ H,
                              const real* __restrict__ K, int nx, int ny, int nz, real dx,
                              uint8_t bc_type, real H_bc) {
 ...
 */
__global__ void kernel_U_east(real* __restrict__ U, const real* __restrict__ H,
                              const real* __restrict__ K, int nx, int ny, int nz, real dx,
                              uint8_t bc_type, real H_bc) {
    int j = threadIdx.x + blockIdx.x * blockDim.x;
    int k = threadIdx.y + blockIdx.y * blockDim.y;
 ...
 */
__global__ void kernel_V_south(real* __restrict__ V, const real* __restrict__ H,
                               const real* __restrict__ K, int nx, int ny, int nz, real dy,
                               uint8_t bc_type, real H_bc) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int k = threadIdx.y + blockIdx.y * blockDim.y;
 ...
 */
__global__ void kernel_V_north(real* __restrict__ V, const real* __restrict__ H,
                               const real* __restrict__ K, int nx, int ny, int nz, real dy,
                               uint8_t bc_type, real H_bc) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int k = threadIdx.y + blockIdx.y * blockDim.y;
 ...
 */
__global__ void kernel_W_bottom(real* __restrict__ W, const real* __restrict__ H,
                                const real* __restrict__ K, int nx, int ny, int nz, real dz,
                                uint8_t bc_type, real H_bc) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
 ...
 */
__global__ void kernel_W_top(real* __restrict__ W, const real* __restrict__ H,
                             const real* __restrict__ K, int nx, int ny, int nz, real dz,
                             uint8_t bc_type, real H_bc) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void kernel_U_interior_padded(real* __restrict__ U, const real* __restrict__ H,
                                         const real* __restrict__ K, int nx, int ny, int nz,
                                         real dx) {
    int face_i = 1 + threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    int k = threadIdx.z + blockIdx.z * blockDim.z;
...
__global__ void kernel_V_interior_padded(real* __restrict__ V, const real* __restrict__ H,
                                         const real* __restrict__ K, int nx, int ny, int nz,
                                         real dy) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int face_j = 1 + threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void kernel_W_interior_padded(real* __restrict__ W, const real* __restrict__ H,
                                         const real* __restrict__ K, int nx, int ny, int nz,
                                         real dz) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void kernel_U_west_padded(real* __restrict__ U, const real* __restrict__ H,
                                     const real* __restrict__ K, int nx, int ny, int nz, real dx,
                                     uint8_t bc_type, real H_bc) {
    int j = threadIdx.x + blockIdx.x * blockDim.x;
    int k = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void kernel_U_east_padded(real* __restrict__ U, const real* __restrict__ H,
                                     const real* __restrict__ K, int nx, int ny, int nz, real dx,
                                     uint8_t bc_type, real H_bc) {
    int j = threadIdx.x + blockIdx.x * blockDim.x;
    int k = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void kernel_V_south_padded(real* __restrict__ V, const real* __restrict__ H,
                                      const real* __restrict__ K, int nx, int ny, int nz, real dy,
                                      uint8_t bc_type, real H_bc) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int k = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void kernel_V_north_padded(real* __restrict__ V, const real* __restrict__ H,
                                      const real* __restrict__ K, int nx, int ny, int nz, real dy,
                                      uint8_t bc_type, real H_bc) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int k = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void kernel_W_bottom_padded(real* __restrict__ W, const real* __restrict__ H,
                                       const real* __restrict__ K, int nx, int ny, int nz, real dz,
                                       uint8_t bc_type, real H_bc) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void kernel_W_top_padded(real* __restrict__ W, const real* __restrict__ H,
                                    const real* __restrict__ K, int nx, int ny, int nz, real dz,
                                    uint8_t bc_type, real H_bc) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
...
void compute_velocity_from_head(VelocityField& vel, const HeadField& head, const KField& K,
    ...
    // U interior: faces i = 1 to nx-1
    {
        ...
        dim3 grid_U((num_faces_x + block.x - 1) / block.x, (ny + block.y - 1) / block.y,
    ...
    // V interior: faces j = 1 to ny-1
    {
        ...
        dim3 grid_V((nx + block.x - 1) / block.x, (num_faces_y + block.y - 1) / block.y,
                    (nz + block.z - 1) / block.z);
    ...
    // W interior: faces k = 1 to nz-1
    {
        ...
        dim3 grid_W((nx + block.x - 1) / block.x, (ny + block.y - 1) / block.y,
...
void compute_velocity_from_head(PaddedVelocityField& vel, const HeadField& head, const KField& K,
    ...
    {
        int nf = nx - 1;
        dim3 g((nf + block.x - 1) / block.x, (ny + block.y - 1) / block.y,
               (nz + block.z - 1) / block.z);
    ...
    {
        int nf = ny - 1;
        dim3 g((nx + block.x - 1) / block.x, (nf + block.y - 1) / block.y,
               (nz + block.z - 1) / block.z);
    ...
    {
        int nf = nz - 1;
        dim3 g((nx + block.x - 1) / block.x, (ny + block.y - 1) / block.y,
    ...
    // U boundaries
    {
        dim3 g((ny + block2D.x - 1) / block2D.x, (nz + block2D.y - 1) / block2D.y);
    ...
    // V boundaries
    {
        dim3 g((nx + block2D.x - 1) / block2D.x, (nz + block2D.y - 1) / block2D.y);
    ...
    // W boundaries
    {
        dim3 g((nx + block2D.x - 1) / block2D.x, (ny + block2D.y - 1) / block2D.y);
...
real compute_norm2(DeviceSpan<const real> data, CudaContext& ctx) {
    ...
    kernel_sum_sq<<<num_blocks, block_size, block_size * sizeof(real)>>>(data.data(),
                                                                         partial.data(), n);
...
real compute_sum(DeviceSpan<const real> data, CudaContext& ctx) {
    ...
    kernel_sum<<<num_blocks, block_size, block_size * sizeof(real)>>>(data.data(), partial.data(),
                                                                      n);
...
real compute_harmonic_mean_K(DeviceSpan<const real> K, CudaContext& ctx) {
    ...
    kernel_sum_inv<<<num_blocks, block_size, block_size * sizeof(real)>>>(K.data(), partial.data(),
                                                                          n);
...
void verify_mean_velocity_darcy(const VelocityField& vel, const KField& K, const Grid3D& grid,
    ...
    // Relative errors
    real rel_err_harmonic =
        std::abs(U_mean - u_theory_harmonic) / std::abs(u_theory_harmonic) * 100.0;
    real rel_err_arithmetic =
        std::abs(U_mean - u_theory_arithmetic) / std::abs(u_theory_arithmetic) * 100.0;
    ...
    std::cout
        << "    The computed mean velocity should be consistent with the solved head field.\n";
}
