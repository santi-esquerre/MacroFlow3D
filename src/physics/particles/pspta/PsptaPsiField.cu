...
int idx3(int i, int j, int k, int nx, int ny) {
    return i + nx * (j + ny * k);
}

/// CompactMAC U-face index: i + (nx+1)*j + (nx+1)*ny*k
__device__ __forceinline__
int idxU(int i, int j, int k, int nx, int ny) {
    return i + (nx + 1) * j + (nx + 1) * ny * k;
}

/// CompactMAC V-face index: i + nx*j + nx*(ny+1)*k
__device__ __forceinline__
int idxV(int i, int j, int k, int nx, int ny_p1, int nx_) {
    // ny_p1 = ny+1
...
__device__ __forceinline__
int idxW(int i, int j, int k, int nx_, int ny_) {
 ...
 */
__global__
void kernel_init_inlet(float* __restrict__ psi1,
 ...
 */
__global__
void kernel_advance_plane(float* __restrict__        psi1,
                          float* __restrict__        psi2,
                          ...
                          double eps_vx,
                          int    i_plane,
                          int* __restrict__          vx_clamped_cnt)
{
    const int j = static_cast<int>(blockIdx.x) * blockDim.x + threadIdx.x;
    const int k = static_cast<int>(blockIdx.y) * blockDim.y + threadIdx.y;
    if (j >= ny || k >= nz) return;
    ...
    // Valid for i_plane <= nx-2: U(i+2,j,k) accesses ix=i+2 <= nx (U has nx+1 faces).
    ...
    double p1 = (1.0 - s) * ((1.0 - t) * p1_00 + t * p1_10)
              +        s  * ((1.0 - t) * p1_01 + t * p1_11);
    ...
    double p2 = (1.0 - s) * ((1.0 - t) * p2_00 + t * p2_10)
              +        s  * ((1.0 - t) * p2_01 + t * p2_11);
 ...
 */
__global__
void kernel_vdotgradpsi(const float* __restrict__ psi1,
                        ...
                        float* __restrict__ r2)
{
    ...
    if (i == 0) {
        dpsi1_dx = (static_cast<double>(psi1[idx3(1, j, k, nx, ny)]) -
                    static_cast<double>(psi1[idx3(0, j, k, nx, ny)])) / dx;
        dpsi2_dx = (static_cast<double>(psi2[idx3(1, j, k, nx, ny)]) -
                    static_cast<double>(psi2[idx3(0, j, k, nx, ny)])) / dx;
    } else if (i == nx - 1) {
        dpsi1_dx = (static_cast<double>(psi1[idx3(nx-1, j, k, nx, ny)]) -
                    static_cast<double>(psi1[idx3(nx-2, j, k, nx, ny)])) / dx;
        dpsi2_dx = (static_cast<double>(psi2[idx3(nx-1, j, k, nx, ny)]) -
                    static_cast<double>(psi2[idx3(nx-2, j, k, nx, ny)])) / dx;
    } else {
        dpsi1_dx = (static_cast<double>(psi1[idx3(i+1, j, k, nx, ny)]) -
                    static_cast<double>(psi1[idx3(i-1, j, k, nx, ny)])) / (2.0 * dx);
        dpsi2_dx = (static_cast<double>(psi2[idx3(i+1, j, k, nx, ny)]) -
                    static_cast<double>(psi2[idx3(i-1, j, k, nx, ny)])) / (2.0 * dx);
    }

    // ── ∂ψ/∂y: periodic central ─────────────────────────────────────────────
    const int jp1 = (j + 1) % ny;
    const int jm1 = (j - 1 + ny) % ny;
    const double dpsi1_dy = (static_cast<double>(psi1[idx3(i, jp1, k, nx, ny)]) -
                              static_cast<double>(psi1[idx3(i, jm1, k, nx, ny)])) / (2.0 * dy);
    const double dpsi2_dy = (static_cast<double>(psi2[idx3(i, jp1, k, nx, ny)]) -
                              static_cast<double>(psi2[idx3(i, jm1, k, nx, ny)])) / (2.0 * dy);
    ...
    const double dpsi1_dz = (static_cast<double>(psi1[idx3(i, j, kp1, nx, ny)]) -
                              static_cast<double>(psi1[idx3(i, j, km1, nx, ny)])) / (2.0 * dz);
    const double dpsi2_dz = (static_cast<double>(psi2[idx3(i, j, kp1, nx, ny)]) -
                              static_cast<double>(psi2[idx3(i, j, km1, nx, ny)])) / (2.0 * dz);

    // ── Residuals ────────────────────────────────────────────────────────────
    r1[idx3(i, j, k, nx, ny)] = static_cast<float>(
        vx * dpsi1_dx + vy * dpsi1_dy + vz * dpsi1_dz);
    r2[idx3(i, j, k, nx, ny)] = static_cast<float>(
        vx * dpsi2_dx + vy * dpsi2_dy + vz * dpsi2_dz);
}
...
PsptaPrecomputeReport PsptaPsiField::precompute_levelA(
    ...
    double               eps_vx)
{
    ...
    for (int i = 0; i < nx_ - 1; ++i) {
        kernel_advance_plane<<<grid2d, block2d, 0, stream>>>(
            psi1.data(), psi2.data(),
            vel.U.data(), vel.V.data(), vel.W.data(),
            nx_, ny_, nz_,
            dx_, dy_, dz_,
            Ly, Lz,
            eps_vx,
            i,
            d_vx_clamped_counter_.data());
        MACROFLOW3D_CUDA_CHECK(cudaGetLastError());
    }
    ...
    int h_cnt = 0;
    MACROFLOW3D_CUDA_CHECK(
        cudaMemcpy(&h_cnt, d_vx_clamped_counter_.data(), sizeof(int),
...
void PsptaPsiField::compute_vdotgradpsi_norms(
    const VelocityField& vel,
    float*               out_r1,
    float*               out_r2,
    cudaStream_t         stream) const
{
    ...
    const dim3 grid3d(static_cast<unsigned>((nx + 7) / 8),
                      static_cast<unsigned>((ny + 7) / 8),
                      static_cast<unsigned>((nz + 7) / 8));

    kernel_vdotgradpsi<<<grid3d, block3d, 0, stream>>>(
        psi1.data(), psi2.data(),
        vel.U.data(), vel.V.data(), vel.W.data(),
        nx, ny, nz,
        dx, dy, dz,
...
__device__ __forceinline__
void atomic_max_double(double* addr, double val) {
 ...
 * ∇ψ: central FDs with one-sided at x-boundaries; periodic-lifted central in y,z.
 *
 * Output buf[5] (device double array):
 *   [0] sumsq1  [1] sumsq2  [2] maxabs1  [3] maxabs2  [4] count (as double)
 */
__global__
void kernel_psi_quality_reduce(
    ...
    double* __restrict__ out_buf)  // 5 doubles initialized to 0
{
    ...
    if (c < total) {
        ...
        const auto idx_W = [&](int ii, int jj, int kk) {
        ...
        const auto idx_psi = [&](int ii, int jj, int kk) {
        ...
        if (i == 0)
            ...
            dp1_dx = (static_cast<double>(psi1_buf[idx_psi(i+1, j, k)]) -
                      static_cast<double>(psi1_buf[idx_psi(i-1, j, k)])) / (2.0 * dx);
        ...
        const double dp1_dz = (static_cast<double>(psi1_buf[idx_psi(i, j, k_p)]) -
                                static_cast<double>(psi1_buf[idx_psi(i, j, k_m)])) / (2.0 * dz);
        ...
        if (i == 0)
            ...
            dp2_dx = (static_cast<double>(psi2_buf[idx_psi(i+1, j, k)]) -
                      static_cast<double>(psi2_buf[idx_psi(i-1, j, k)])) / (2.0 * dx);

        const double dp2_dy = (static_cast<double>(psi2_buf[idx_psi(i, j_p, k)]) -
                                static_cast<double>(psi2_buf[idx_psi(i, j_m, k)])) / (2.0 * dy);
    ...
    for (int stride = bs / 2; stride > 0; stride >>= 1) {
        if (tid < stride) {
            ...
            if (s_max1[tid + stride] > s_max1[tid]) s_max1[tid] = s_max1[tid + stride];
            if (s_max2[tid + stride] > s_max2[tid]) s_max2[tid] = s_max2[tid + stride];
        }
        __syncthreads();
    }
    ...
    if (tid == 0) {
        ...
        atomic_add_double(&out_buf[4], static_cast<double>(min(bs, total - (int)(blockIdx.x * bs))));
    }
}

// ── Host method ──────────────────────────────────────────────────────────────

PsiQualityReport PsptaPsiField::compute_psi_quality(
    const VelocityField& vel,
    const Grid3D&        grid,
    cudaStream_t         stream) const
{
    ...
    MACROFLOW3D_CUDA_CHECK(
        cudaMemsetAsync(d_psi_quality_buf_.data(), 0,
                        5 * sizeof(double), stream));
    ...
    kernel_psi_quality_reduce<<<nblocks, block_sz, smem, stream>>>(
        ...
        Ly, Lz,
        d_psi_quality_buf_.data());
    MACROFLOW3D_CUDA_CHECK(cudaGetLastError());

    double host_buf[5] = {};
    MACROFLOW3D_CUDA_CHECK(
        cudaMemcpyAsync(host_buf, d_psi_quality_buf_.data(),
                        5 * sizeof(double), cudaMemcpyDeviceToHost, stream));
    MACROFLOW3D_CUDA_CHECK(cudaStreamSynchronize(stream));
