...
__device__ __forceinline__ double clamp_d(double v, double lo, double hi) {
...
__device__ __forceinline__ uint64_t hash64(uint64_t k) {
    k ^= k >> 33;
...
__device__ __forceinline__ float hash_to_f01(uint64_t seed, uint64_t salt) {
...
__device__ __forceinline__ double wrap_to_L(double x, double L) {
    return x - floor(x / L) * L;
}

/// Nearest-image residual: r ∈ (-L/2, L/2]
__device__ __forceinline__ double wrap_diff(double f, double L) {
...
__device__ __forceinline__ int idx_U(int i, int j, int k, int nx, int ny) {
    return i + (nx + 1) * j + (nx + 1) * ny * k;
}

/// V index: i + nx*j + nx*(ny+1)*k
__device__ __forceinline__ int idx_V(int i, int j, int k, int nx_v, int nyp1) {
    return i + nx_v * j + nx_v * nyp1 * k;
}

/// W index: i + nx*j + nx*ny*k
__device__ __forceinline__ int idx_W(int i, int j, int k, int nx_w, int ny_w) {
    return i + nx_w * j + nx_w * ny_w * k;
}

/// Cell-centered ψ index (x-fastest): i + nx*(j + ny*k)
__device__ __forceinline__ int idx_psi(int i, int j, int k, int nx, int ny) {
    return i + nx * (j + ny * k);
}
...
__device__ double
sample_psi_and_partials(const float* __restrict__ psi_buf, double x, double y, double z, int nx,
                        int ny, int nz, double dx, double dy, double dz,
                        double L_period_y, ///< period in y dimension (Ly for ψ1, Ly for ψ2 too
                                           ///< since x-fastest)
    ...
    // ── ψ value ────────────────────────────────────────────────────────────
    const double psi = c000 * tx0 * ty0 * tz0 + c100 * tx1 * ty0 * tz0 + c010 * tx0 * ty1 * tz0 +
                       c110 * tx1 * ty1 * tz0 + c001 * tx0 * ty0 * tz1 + c101 * tx1 * ty0 * tz1 +
...
__device__ __forceinline__ double sample_vx(const real* __restrict__ U, double x, double y,
                                            double z, int nx, int ny, int nz, double dx, double dy,
                                            double dz) {
    ...
    return u000 * (1 - tx) * (1 - ty) * (1 - tz) + u100 * tx * (1 - ty) * (1 - tz) +
           ...
           u001 * (1 - tx) * (1 - ty) * tz + u101 * tx * (1 - ty) * tz + u011 * (1 - tx) * ty * tz +
           u111 * tx * ty * tz;
}
...
__device__ bool newton_solve_yz(const float* __restrict__ psi1_buf,
                                const float* __restrict__ psi2_buf, double x, double psi1_c,
                                double psi2_c, double y0, double z0, int nx, int ny, int nz,
    ...
    // Float32 ψ storage limits resolution to ~1e-4 * cell, so 1e-4 is
    // appropriate.
    ...
    for (int it = 0; it < PSPTA_N_NEWTON; ++it) {
        ...
        const double p1 = sample_psi_and_partials(psi1_buf, x, y, z, nx, ny, nz, dx, dy, dz, Ly, Lz,
                                                  Ly, &dp1_dy, &dp1_dz);

        const double p2 = sample_psi_and_partials(psi2_buf, x, y, z, nx, ny, nz, dx, dy, dz, Ly, Lz,
                                                  Lz, &dp2_dy, &dp2_dz);
    ...
    *z_out = z;
    ...
    // (Avoids extra EvalPsiAndPartials; use sample_psi_and_partials with dummy
    // partials.)
    double dp1_dy_dummy, dp1_dz_dummy;
    const double p1_fin = sample_psi_and_partials(psi1_buf, x, y, z, nx, ny, nz, dx, dy, dz, Ly, Lz,
                                                  Ly, &dp1_dy_dummy, &dp1_dz_dummy);
    double dp2_dy_dummy, dp2_dz_dummy;
    const double p2_fin = sample_psi_and_partials(psi2_buf, x, y, z, nx, ny, nz, dx, dy, dz, Ly, Lz,
                                                  Lz, &dp2_dy_dummy, &dp2_dz_dummy);
...
__global__ void kernel_inject_box(real* __restrict__ px, real* __restrict__ py,
                                  ...
                                  int32_t* wrapX, int32_t* wrapY, int32_t* wrapZ, int first,
                                  int total, real x0, real y0, real z0, real x_range, real y_range,
                                  real z_range, uint64_t seed) {
...
/**
 * @brief Sample ψ1, ψ2 at each particle's position; store as per-particle
 * constants.
 *
 ...
 */
__global__ void
kernel_init_invariants(const real* __restrict__ px, const real* __restrict__ py,
                       const real* __restrict__ pz, const uint8_t* __restrict__ status,
                       const float* __restrict__ psi1_buf, const float* __restrict__ psi2_buf,
                       float* __restrict__ psi1_const, float* __restrict__ psi2_const,
                       float* __restrict__ y_guess, float* __restrict__ z_guess, int N, int nx,
                       int ny, int nz, double dx, double dy, double dz, double Ly, double Lz) {
    ...
    const double p1_val = sample_psi_and_partials(psi1_buf, x, y, z, nx, ny, nz, dx, dy, dz, Ly, Lz,
                                                  Ly, &dp1_dy, &dp1_dz);

    // ψ2 — self-period is Lz
    double dp2_dy, dp2_dz;
    const double p2_val = sample_psi_and_partials(psi2_buf, x, y, z, nx, ny, nz, dx, dy, dz, Ly, Lz,
                                                  Lz, &dp2_dy, &dp2_dz);
 ...
 *   6. Wrap x_new; detect y,z periodic crossings via wrap_diff; update
 * wrapX/Y/Z
 ...
 * Thread layout: 1D over particles [0, N).
 */
__global__ void kernel_pspta_step(real* __restrict__ px, real* __restrict__ py,
                                  real* __restrict__ pz, uint8_t* __restrict__ status,
                                  int32_t* wrapX, int32_t* wrapY, int32_t* wrapZ,
                                  ...
                                  const float* __restrict__ psi2_const, float* __restrict__ y_guess,
                                  float* __restrict__ z_guess, uint32_t* __restrict__ fail_count,
                                  int N, int nx, int ny, int nz, double dx, double dy, double dz,
                                  double Lx, double Ly, double Lz, double dt, bool x_periodic) {
    int p = static_cast<int>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (p >= N)
        return;
    if (status[p] != 0)
    ...
    // ── 1. Project at x ───────────────────────────────────────────────────
    double y0, z0;
    const bool ok0 = newton_solve_yz(psi1_buf, psi2_buf, x, pc1, pc2, yg, zg, nx, ny, nz, dx, dy,
                                     dz, Ly, Lz, &y0, &z0);
    ...
    // ── 3. Project at x_mid ───────────────────────────────────────────────
    double y_mid, z_mid;
    const bool ok_mid = newton_solve_yz(psi1_buf, psi2_buf, x_mid, pc1, pc2, y0, z0, nx, ny, nz, dx,
                                        dy, dz, Ly, Lz, &y_mid, &z_mid);
    ...
    const double vx_mid = sample_vx(U, x_mid, y_mid, z_mid, nx, ny, nz, dx, dy, dz);
    // Fix D: vx_mid ≤ 0 or non-finite → do nothing.
    if (vx_mid <= 0.0 || !isfinite(vx_mid)) {
        fail_count[p]++;
        return;
    }
    ...
    double y_new, z_new;
    const bool ok_new = newton_solve_yz(psi1_buf, psi2_buf, x_new_w, pc1, pc2, y_mid, z_mid, nx, ny,
                                        nz, dx, dy, dz, Ly, Lz, &y_new, &z_new);
    ...
    // round() gives the exact integer winding number change (robust for |disp| <
    // Ly/2).
    ...
    if (wrapX && cross_x != 0)
        wrapX[p] += cross_x;
    if (wrapY && delta_wrapY != 0)
        wrapY[p] += delta_wrapY;
    if (wrapZ && delta_wrapZ != 0)
        wrapZ[p] += delta_wrapZ;
}

// ============================================================================
// B.4  Kernel: count_stats  (diagnostics — called after transport, not in hot
// loop)
// ============================================================================
 ...
 */
__global__ void kernel_count_stats(const uint8_t* __restrict__ status,
    ...
    if (s == 0)
        atomicAdd(&out[0], 1ULL);
    else if (s == PSPTA_STATUS_EXITED)
        atomicAdd(&out[1], 1ULL);
    else
        atomicAdd(&out[2], 1ULL);
 ...
 */
__device__ __forceinline__ int pspta_fail_bin(uint32_t f) {
    if (f == 0)
        return 0;
    if (f == 1)
        return 1;
 ...
 */
__global__ void kernel_fail_details(const uint32_t* __restrict__ fail_count,
...
__global__ void kernel_compute_unwrapped(const real* __restrict__ px, const real* __restrict__ py,
                                         ...
                                         const int32_t* __restrict__ wrapZ, real* __restrict__ ux,
                                         real* __restrict__ uy, real* __restrict__ uz, int N,
...
PsptaEngine::PsptaEngine(const Grid3D& grid, cudaStream_t stream, uint64_t inject_seed)
    : grid_(grid), Lx_(static_cast<double>(grid.nx) * static_cast<double>(grid.dx)),
      Ly_(static_cast<double>(grid.ny) * static_cast<double>(grid.dy)),
      Lz_(static_cast<double>(grid.nz) * static_cast<double>(grid.dz)), stream_(stream),
...
void PsptaEngine::bind_psifield(const PsptaPsiField* psi) {
    psi_ = psi;
    // Populate unified binding struct for internal use
    if (psi) {
        psi_bind_.psi1 = psi->psi1_ptr();
        psi_bind_.psi2 = psi->psi2_ptr();
        psi_bind_.nx = psi->nx;
        psi_bind_.ny = psi->ny;
        psi_bind_.nz = psi->nz;
        psi_bind_.dx = psi->dx;
        psi_bind_.dy = psi->dy;
        psi_bind_.dz = psi->dz;
        psi_bind_.valid = true;
    } else {
        psi_bind_ = PsiBinding{};
    }
}

void PsptaEngine::bind_invariants(const PsptaInvariantField* inv) {
    // Populate unified binding struct from new interface
    if (inv && inv->is_valid()) {
        psi_bind_.psi1 = inv->psi1_ptr();
        psi_bind_.psi2 = inv->psi2_ptr();
        psi_bind_.nx = inv->nx();
        psi_bind_.ny = inv->ny();
        psi_bind_.nz = inv->nz();
        psi_bind_.dx = inv->dx();
    ...
    // Clear legacy pointer when using new interface
    psi_ = nullptr;
}
...
void PsptaEngine::inject_box(real x0, real y0, real z0, real x1, real y1, real z1, int first,
                             int count) {
    if (count <= 0)
        return;
    ...
    kernel_inject_box<<<grid, block, 0, stream_>>>(
        ...
        first, count, x0, y0, z0, x_range, y_range, z_range, inject_seed_);
...
void PsptaEngine::prepare() {
    ...
    // Zero fail counters
    MACROFLOW3D_CUDA_CHECK(cudaMemsetAsync(d_fail_count_.data(), 0,
    ...
    kernel_init_invariants<<<grid_k, block, 0, stream_>>>(
        ...
        d_psi1_const_.data(), d_psi2_const_.data(), d_y_guess_.data(), d_z_guess_.data(), N,
        psi_bind_.nx, psi_bind_.ny, psi_bind_.nz, psi_bind_.dx, psi_bind_.dy, psi_bind_.dz, Ly_,
        Lz_);
    MACROFLOW3D_CUDA_CHECK(cudaGetLastError());
}
...
void PsptaEngine::step(real dt) {
    ...
    kernel_pspta_step<<<grid_k, block, 0, stream_>>>(
        parts_.x, parts_.y, parts_.z, parts_.status, parts_.wrapX, parts_.wrapY, parts_.wrapZ,
        psi_bind_.psi1, psi_bind_.psi2, vel_->U.data(), d_psi1_const_.data(), d_psi2_const_.data(),
        d_y_guess_.data(), d_z_guess_.data(), d_fail_count_.data(), N, grid_.nx, grid_.ny, grid_.nz,
        static_cast<double>(grid_.dx), static_cast<double>(grid_.dy), static_cast<double>(grid_.dz),
        Lx_, Ly_, Lz_, static_cast<double>(dt), x_periodic_);
    // No MACROFLOW3D_CUDA_CHECK here to keep the path non-synchronizing in
    // release. The pipeline's single sync point handles error detection.
}
...
void PsptaEngine::compute_unwrapped(UnwrappedSoA<real>& uw, cudaStream_t stream) {
    ...
    kernel_compute_unwrapped<<<grid_k, block, 0, stream>>>(
        parts_.x, parts_.y, parts_.z, parts_.wrapX, parts_.wrapY, parts_.wrapZ, uw.x_u, uw.y_u,
        uw.z_u, N, Lx_, Ly_, Lz_);
    MACROFLOW3D_CUDA_CHECK(cudaGetLastError());
}
...
PsptaEngine::TransportStats PsptaEngine::compute_transport_stats() {
    ...
    if (d_stats_buf_.size() < 4)
        d_stats_buf_.resize(4);
    MACROFLOW3D_CUDA_CHECK(
        cudaMemsetAsync(d_stats_buf_.data(), 0, 4 * sizeof(unsigned long long), stream_));

    kernel_count_stats<<<grid_k, block, 0, stream_>>>(parts_.status, d_fail_count_.data(),
                                                      d_stats_buf_.data(), N);
    MACROFLOW3D_CUDA_CHECK(cudaGetLastError());

    unsigned long long host_buf[4] = {0, 0, 0, 0};
    MACROFLOW3D_CUDA_CHECK(cudaMemcpyAsync(host_buf, d_stats_buf_.data(),
    ...
    if (d_fail_detail_buf_.size() < DETAIL_SIZE)
        d_fail_detail_buf_.resize(DETAIL_SIZE);
    MACROFLOW3D_CUDA_CHECK(
        cudaMemsetAsync(d_fail_detail_buf_.data(), 0, DETAIL_SIZE * sizeof(uint32_t), stream_));

    kernel_fail_details<<<grid_k, block, 0, stream_>>>(d_fail_count_.data(),
                                                       d_fail_detail_buf_.data(), N);
    MACROFLOW3D_CUDA_CHECK(cudaGetLastError());

    uint32_t detail_buf[DETAIL_SIZE] = {};
    MACROFLOW3D_CUDA_CHECK(cudaMemcpyAsync(detail_buf, d_fail_detail_buf_.data(),
                                           DETAIL_SIZE * sizeof(uint32_t), cudaMemcpyDeviceToHost,
                                           stream_));
