...
struct Par2StatsAdapter::Impl {
    ...
    Impl(int max_particles, const par2::GridDesc<real>& g, bool periodic, bool biased)
        : stats(max_particles), grid(g), has_periodic(periodic), use_biased(biased) {
...
Par2StatsAdapter::Par2StatsAdapter(int max_particles, const Grid3D& grid, bool has_periodic_bc,
                                   bool use_biased_var)
    : impl_(std::make_unique<Impl>(max_particles, make_par2_grid(grid), has_periodic_bc,
                                   use_biased_var)) {}

Par2StatsAdapter::~Par2StatsAdapter() = default;

void Par2StatsAdapter::sample_async(const ConstParticlesSoA<real>& particles, cudaStream_t stream) {
...
bool Par2StatsAdapter::write_csv(const std::string& filename) const {
    ...
    for (const auto& s : impl_->samples) {
        std::fprintf(f, "%.15e,%.15e,%.15e,%.15e,%.15e,%.15e,%.15e,%d\n", (double)s.time,
                     (double)s.mean[0], (double)s.mean[1], (double)s.mean[2], (double)s.var[0],
                     (double)s.var[1], (double)s.var[2], s.active);
    }
    std::fclose(f);
    return true;
}
