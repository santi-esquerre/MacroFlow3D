...
template <typename T> class DeviceSpan {
  public:
    ...
    template <typename U = T, typename = typename std::enable_if<std::is_const<U>::value &&
                                                                 std::is_same<U, T>::value>::type>
    __host__ __device__ DeviceSpan(const DeviceSpan<typename std::remove_const<U>::type>& other)
        : ptr_(other.data()), n_(other.size()) {}

    __host__ __device__ T* data() const { return ptr_; }

    __host__ __device__ size_t size() const { return n_; }

    __host__ __device__ bool empty() const { return n_ == 0; }

    // Device-only to prevent dangerous host access to device memory
    __device__ T& operator[](size_t i) const { return ptr_[i]; }
