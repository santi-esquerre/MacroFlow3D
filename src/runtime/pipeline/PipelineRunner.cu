...
void print_config_summary(const AppConfig& cfg) {
    ...
    std::printf("  Grid:        %d × %d × %d  (dx=%.6f)\n", cfg.grid.nx, cfg.grid.ny, cfg.grid.nz,
                cfg.grid.dx);
    std::printf("  Domain:      [0, %.4f] × [0, %.4f] × [0, %.4f]\n", cfg.grid.Lx(), cfg.grid.Ly(),
                cfg.grid.Lz());
    std::printf("  Stochastic:  σ²=%.2f  λ=%.4f  modes=%d  seed=%lu\n", cfg.stochastic.sigma2,
                cfg.stochastic.corr_length, cfg.stochastic.n_modes,
                (unsigned long)cfg.stochastic.seed);
    ...
    if (mac.enabled) {
        std::printf(
            "  Analysis:    NR=%d  λ_macro=%.4f  ||<v>||=%.4f  sample_every=%d  estimator=%s\n",
            mac.NR, mac.lambda, mac.vmean_norm, mac.sample_every, mac.var_estimator.c_str());
    } else {
        std::printf("  Analysis:    macrodispersion OFF (single realization)\n");
    }
    const auto& snap = cfg.analysis.snapshots;
    if (snap.enabled) {
        std::printf("  Snapshots:   every=%d  stride=%d  unwrapped=%s\n", snap.every, snap.stride,
                    snap.include_unwrapped ? "yes" : "no");
    }
...
int run_pipeline(const AppConfig& cfg, CudaContext& ctx, StageProfiler& profiler) {
