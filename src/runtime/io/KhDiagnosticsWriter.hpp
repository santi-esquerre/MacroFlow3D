...
class KhDiagnosticsWriter {
  public:
    static void write_field_row(const std::string& path,
        ...
        if (write_header) {
            f << "realization_id,backend,n_samples,sample_stride,finite_count,invalid_count,"
                 ...
                 "helicity_abs_mean,helicity_abs_max,helicity_norm_mean,helicity_norm_p95,"
                 "helicity_norm_max\n";
        }
        f << s.realization_id << ',' << s.backend << ',' << s.n_samples << ',' << s.sample_stride
          ...
          << ',' << s.curl_mag_max << ',' << s.helicity_abs_mean << ',' << s.helicity_abs_max << ','
          << s.helicity_norm_mean << ',' << s.helicity_norm_p95 << ',' << s.helicity_norm_max
          << '\n';
    }
    ...
    static void write_comparison_row(const std::string& path,
        ...
        auto f = kh_detail::open_append(path);
        if (write_header) {
            f << "realization_id,n_samples,sample_stride,finite_count,invalid_count,"
                 "rel_l2_diff,diff_mean,diff_std,diff_p50,diff_p95,diff_max,"
                 "vector_correlation\n";
        }
        f << s.realization_id << ',' << s.n_samples << ',' << s.sample_stride << ','
          << s.finite_count << ',' << s.invalid_count << ',' << s.rel_l2_diff << ',' << s.diff_mean
          << ',' << s.diff_std << ',' << s.diff_p50 << ',' << s.diff_p95 << ',' << s.diff_max << ','
