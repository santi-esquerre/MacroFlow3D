...
class StageProfiler {
public:
    explicit StageProfiler(CudaContext& ctx) 
        : ctx_(ctx), enabled_(MACROFLOW3D_PROFILING_ENABLED) {
    ...
    void stop() {
#if MACROFLOW3D_ENABLE_PROFILING
        if (!enabled_ || current_stage_.empty()) return;
    ...
    void report() const {
        ...
        for (const auto& s : stages_) {
            ...
            std::printf("║ %-25s │ %8.2f │ %5d │ %8.2f │ %5.1f ║\n",
                       s.name.c_str(), s.total_ms, s.count, avg_ms, pct);
        }
        ...
        std::printf("║ GPU Memory: %.1f MB used / %.1f MB total (%.1f%% used)      ║\n",
                    used_mb, total_mb, (used_mb / total_mb * 100.0f));
        std::printf("╚═══════════════════════════════════════════════════════════════╝\n\n");
#endif
    }
...
#else
#  define MACROFLOW3D_PROFILE_STAGE(profiler, name) do {} while(0)
#endif
