...
CGResult cg_solve(CudaContext& ctx, const Operator& A, DeviceSpan<const real> b, DeviceSpan<real> x,
                  const CGConfig& cfg, CGWorkspace& ws) {
    ...
    if (cfg.verbose) {
        ...
        MACROFLOW3D_CUDA_CHECK(
            cudaMemcpy(&b_norm, ws.d_rr.data(), sizeof(real), cudaMemcpyDeviceToHost));
        blas::dot_device(ctx, r_span, r_span, d_rr_span, ws.red);
        MACROFLOW3D_CUDA_CHECK(
            cudaMemcpy(&Ax_norm, ws.d_rr.data(), sizeof(real), cudaMemcpyDeviceToHost));
        ctx.synchronize();
        std::cout << "[CG DEBUG] ||b|| = " << std::sqrt(b_norm)
                  << ", ||A*x|| = " << std::sqrt(Ax_norm) << "\n";
    }
    ...
    for (int iter = 0; iter < cfg.max_iter; ++iter) {
        ...
        if (cfg.verbose && iter < 5) {
            ...
            MACROFLOW3D_CUDA_CHECK(
                cudaMemcpy(&rr_debug, ws.d_rr.data(), sizeof(real), cudaMemcpyDeviceToHost));
            MACROFLOW3D_CUDA_CHECK(
                cudaMemcpy(&pAp_debug, ws.d_pAp.data(), sizeof(real), cudaMemcpyDeviceToHost));
            ctx.synchronize();
        ...
        if ((iter + 1) % cfg.check_every == 0) {
            ...
            int is_valid_host;
            MACROFLOW3D_CUDA_CHECK(cudaMemcpyAsync(&is_valid_host, ws.d_is_valid.data(),
