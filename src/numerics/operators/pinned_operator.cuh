 ...
 */
template <typename Op> class PinnedOperator {
  public:
    PinnedOperator(Op& inner, DeviceSpan<const real> K, real dx) : inner_(inner), K_(K), dx_(dx) {}
...
__global__ void kernel_apply_pin(const real* __restrict__ x, real* __restrict__ y,
...
void PinnedOperator<Op>::apply_pin_kernel(CudaContext& ctx, DeviceSpan<const real> x,
                                          DeviceSpan<real> y) const {
    real inv_dx2 = real(1.0) / (dx_ * dx_);
    kernel_apply_pin<<<1, 1, 0, ctx.cuda_stream()>>>(x.data(), y.data(), K_.data(), 0, 0, inv_dx2);
}
