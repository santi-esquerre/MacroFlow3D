    ...
    struct MoveParticle
    {
        ...
        */
                        ...
                        )
        {
            ...
            nY = thrust::raw_pointer_cast(_nY.data());
            nZ = thrust::raw_pointer_cast(_nZ.data());            
        ...
        */
        ...
        {
            unsigned int pid = thrust::get<3>(p); // id de partícula
            int idx, idy, idz;
            grid::idPoint(grid,
                          thrust::get<0>(p),
                          thrust::get<1>(p),
            ...
            dpy = ((vly + vcy)*dt + (B01*xi0 + B11*xi1 + B12*xi2)) ;
            if (grid.nz != 1)
            dpz = ((vlz + vcz)*dt + (B02*xi0 + B12*xi1 + B22*xi2));
            ...
            // Update positions with periodic BC
            thrust::get<0>(p) += dpx;

            // Periodic BC in Y
            thrust::get<1>(p) += dpy;
            if (thrust::get<1>(p) < grid.ymin){
                thrust::get<1>(p) += (grid.ymax - grid.ymin);
                nY[pid] -= 1;
            }
            else if (thrust::get<1>(p) >= grid.ymax){
                thrust::get<1>(p) -= (grid.ymax - grid.ymin);
                nY[pid] += 1;
            }

            // Periodic BC in Z (only if 3D)
            if (grid.nz != 1) {
                thrust::get<2>(p) += dpz;
                if (thrust::get<2>(p) < grid.zmin){
                    thrust::get<2>(p) += (grid.zmax - grid.zmin);
                    nZ[pid] -= 1;
                }
                else if (thrust::get<2>(p) >= grid.zmax){
                    thrust::get<2>(p) -= (grid.zmax - grid.zmin);
                    nZ[pid] += 1;
                }
            }            
