...
| 2026-08-06T02:19Z | closeout validation recovery; base `5874ae3bbc94f9f2285cd79e10a07063a791c050` | Audited the closeout metadata (`5874ae3` changes only dashboard/SF-07 documentation) and confirmed merged SF-07 `e7f90db39bca1906b8bbc5dd94195edf7084cfac` is an ancestor of `master`; its tree and `master` tree are both `bb08fc4da9289fbe840040114353dec5ae63ac40`. T13 PID `107123` and clean-first T13R PID `109253` each detached without a recoverable exit and are invalid—no tests, edits, or commits are attributed to them. T13R2 performed the single authorized clean recovery after `cmake --preset wsl-debug`: `cmake --build build/wsl-debug --clean-first -j1` captured through the tool session/cell mechanism (session `88609`, polled to explicit `exit_code=0`); no relaunch. | Checker before/after PASS (`29 increments, next=SF-08`); configure PASS; clean 102-step build PASS (only existing unused `NZ` and device-long-double warnings). Individually PASS: `gradient_pure_affine` six RMS/Linf=0; `gradient_gpu_oracle` max normalized RMS `9.29285e-17`, boundary Linf `4.29506e-16`; `gradient_smooth_order` minimum L2 order `1.924818` and all Linf decrease; `gradient_error_contract` 24 exact `std::invalid_argument` plus 2 accepted at `9.29285e-17`; mutation sensitivities `.277108/.289053/.160674`. Full streamfunction suite PASS; `run_operator_tests` 8/8 PASS; focused CTest 1/1 (9.56s), full CTest 2/2 (7.85s); smoke `apps/config_pspta_small.yaml` seed 42 grid 64x32x32: head 10 iters `1.02e+01 -> 1.77e-13`, divergence `-8.1175e-14/+8.2979e-14`, particles `387/113`, stalls/nonzero/max fails `0/0/0`. `git diff --check master...HEAD` PASS. Hardware: `asus-santi`, Linux `7.1.5-1-cachyos`, i7-12650H (16 logical CPUs), RTX 3050 Laptop 4096 MiB driver 610.43.03, CUDA 13.3.73, GCC 16.1.1, CMake 4.4.2, preset `wsl-debug` (CUDA arch 86). Gate 1, Gate 2, and Gate 3A operator subset PASS; coupled `r_F`, `e_v`, Darcy-invariance, reconstructed-divergence, denominator/gauge-restoration metrics, full physical Gate 3A, Gates 4/5, and V100 are N/A because SF-07 supplies only the unfused explicit-buffer gradient operator, with no nonlinear sources/solver, reconstruction, production consumer, or remote authorization. | Root audit and closeout PR; do not claim it merged. |
